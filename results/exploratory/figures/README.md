...
(`go_granularity_*`, `go_native_features_*`) in `paper_final/figures/`.

Kept for provenance; **not cited in the manuscript**. All are regenerable from the CSVs
in `paper_final/data/` (and the archived CSVs in `../data/`) via
`scripts/make_figures.py --name <stem>`.
...
The paper's figures live in `paper_final/figures/`.
